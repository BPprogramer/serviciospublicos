...
class ActiveRecord
{
    ...
    public function eliminarLeft($columna, $string, $longitud)
    {

        $query = "DELETE FROM " . static::$tabla . " WHERE LEFT($columna, $longitud) = '$string'";
    ...
    // SUM con condiciones tipo whereArray
    public static function sumWhere($array = [], $columna = 'monto')
    {
        $query = "SELECT SUM($columna) as total FROM " . static::$tabla . " WHERE ";
    ...
    // Ejecutar query crudo y devolver resultado plano
 public static function queryRaw($query)
{
    $resultado = self::$db->query($query);

    if (!$resultado) {
        die("Error SQL: " . self::$db->error);
    }
